...
for id1 in patient_list:
    ...
    structureFile = structureFilePath + str(id1)+ '.h5'
    # #=========================================
    ...
    print('patient', id1)
    print('PTV and BladderOverlap', np.shape(np.intersect1d(np.nonzero(PTVLabel), np.nonzero(bladderLabel))))
    print('PTV and RecOverlap', np.shape(np.intersect1d(np.nonzero(PTVLabel), np.nonzero(rectumLabel))))
    np.savetxt(data_result_path+str(id1)+'PTVBlaOverlap', np.intersect1d(np.nonzero(PTVLabel), np.nonzero(bladderLabel)))
    np.savetxt(data_result_path+str(id1)+'PTVRecOverlap', np.intersect1d(np.nonzero(PTVLabel), np.nonzero(rectumLabel)))
