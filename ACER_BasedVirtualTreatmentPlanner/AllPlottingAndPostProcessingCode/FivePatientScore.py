...
# repList = ['010']
#repList = ['017']
# repList = ['014', '015', '023', '073', '098']
repList = ['030', '080','081','091', '093' ]
...
for i in range(len(repList)):
    ...
    # np.save('/data2/mainul/DataAndGraphDQN/FGSMPaper/PaperRepresantativeUnperturbed', state_test0)
    ScoreSavePath = '/data2/mainul/FivePatientRun/'
    os.makedirs(ScoreSavePath, exist_ok = True)


    planScore_fine, planScore,scoreall = planIQ_train(MPTV, MBLA1, MREC1, xVec,pdose, True)
    print(planScore)
    print(scoreall)
    np.save(ScoreSavePath+str(sampleid)+ 'Score0', planScore)
    np.save(ScoreSavePath+str(sampleid)+ 'Scoreall0', scoreall)
    ...
    # np.save('/data2/mainul/DataAndGraph/PaperRepresantativeBeforeperturbedACER', state_test0)
    planScore_fine, planScore,scoreall = planIQ_train(MPTV, MBLA1, MREC1, xVec,pdose, True)
    print('score afterwards',planScore)
    print('scoreall afterwards', scoreall)
    np.save(ScoreSavePath+str(sampleid)+ 'Score1', planScore)
    np.save(ScoreSavePath+str(sampleid)+ 'Scoreall1', scoreall)
