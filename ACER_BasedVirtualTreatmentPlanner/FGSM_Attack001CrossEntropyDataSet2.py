    ...
    while True:
        if done:
            ...
            if patientid == 0:
                ...
                while (planScore >= 6):
                    ...
                    planscoresSavePath = '/data2/mainul1/results_CORS/FGSM_Attack0.001crossEntropy/scratch6_30StepsNewParamenters3/planscores/'
                    data_result_path='/data2/mainul1/results_CORS/FGSM_Attack0.001crossEntropy/scratch6_30StepsNewParamenters3/dataWithPlanscoreRun/'
                ...
                state_test = First_state + (0.001*gradient.sign())
                print('perturbed state', state_test)
                Y = state_test.detach().numpy()
                data_result_path='/data2/mainul1/results_CORS/FGSM_Attack0.001crossEntropy/scratch6_30StepsNewParamenters3/dataWithPlanscoreRun/'
                planscoresSavePath = '/data2/mainul1/results_CORS/FGSM_Attack0.001crossEntropy/scratch6_30StepsNewParamenters3/planscores/'
                actionSavePath = '/data2/mainul1/results_CORS/FGSM_Attack0.001crossEntropy/scratch6_30StepsNewParamenters3/actions/'
        ...
        if episode_length == 0:
            ...
            for i in range(18):
                j = torch.tensor(i)
                if j != action:
                    second_term = second_term + torch.log(1.0-policy[0,j])
            print('second_term', second_term)
            loss = torch.log(prob)+ second_term
...
def test(rank, args, T):
    ...
    while T.value() <= args.T_max:
        if can_test:
            ...
            actionArray = np.zeros((50,2))
            data_result_path='/data2/mainul1/results_CORS/FGSM_Attack0.001crossEntropy/scratch6_30StepsNewParamenters3/dataWithPlanscoreRun/'
            for rep in range(50):
                disRep = []
                discrepancy = 0
                print("========================================iteration", rep)
                for patientid in range(2):
                    id1 = patientid
                    print("========================================Patient",id1)
                    print("done", done)
