...
def test(rank, args, T):
    ...
    while T.value() <= args.T_max:
        if can_test:
            ...
            for patientid in range(len(patient_list)):
                id1 = patient_list[patientid]
                print("========================================Patient",id1)
                print("done", done)
                ...
                while True:
                    ...
                    if done:
                        ...
                        while planScore >= 8 and whileSteps < 5:
                            # this is the general block
                            whileSteps = whileSteps+1
                            epsilon = 1e-10
                    ...
                    np.save(data_result_path+str(patientid)+'Policy' + str(epoch)  + 'step' + str(episode_length), policySave)
...
async def send_notification():
    await bot.send_message(chat_id=CHAT_ID, text="The Process patient_test_UTSWallrandom6C is Finished")

async def send_notification_error():
    await bot.send_message(chat_id=CHAT_ID, text="The Process patient_test_UTSWallrandom6C is Finished with error")


# The following block is for running in the background==============================
if __name__ == '__main__':
  with open('patient_test_UTSWallrandom6C.txt', 'w') as f:
